...
def stream_text(text: str, speed: float = 0.02):
    """Yield words one by one for st.write_stream animation."""
...
{case.age}
{"M" if case.gender.lower()=="male" else "F"}
</div>
...
if prompt:
    ...
    # Show streaming patient response
    ...
    st.session_state.conversation.extend(
        [f"user:{prompt}", f"patient:{full_answer}"]
    )
